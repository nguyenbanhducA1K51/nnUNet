...
def create_lists_from_splitted_dataset_folder(folder: str, file_ending: str, identifiers: List[str] = None) -> List[
    ...
    for f in identifiers:
        #for each identifier , a patient, can have different modality, thus list_of_lists is a list which each element is a list of differnet modality of image
        # regular expression compile, note escape method https://docs.python.org/3/library/re.html
...
def get_filenames_of_train_images_and_targets(raw_dataset_folder: str, dataset_json: dict = None):
    ...
    # in dataset.json of LiTs does not have dataset keys
    if 'dataset' in dataset_json.keys():
        ...
        # get_identifiers_from_splitted_dataset_folder this get the list of unique patient
        ...
        # this preserve the order (match images with labels) since it all use loop for i in identifiers
        dataset = {i: {'images': im, 'label': se} for i, im, se in zip(identifiers, images, segs)}
    return dataset


if __name__ == '__main__':
    get_filenames_of_train_images_and_targets(join(nnUNet_raw, 'Dataset152_LiTs'))
    # print(get_filenames_of_train_images_and_targets(join(nnUNet_raw, 'Dataset152_LiTs')))
